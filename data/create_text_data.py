...
def createDataset(image_path_list, label_list, outputPath, mode, author_id, remove_punc, resize, imgH, init_gap, h_gap,
    ...
    outputPath = outputPath + (resize == 'charResize') * ('/h%schar%sto%s/' % (imgH, charminW, charmaxW)) + (
                resize == 'keepRatio') * ('/h%s/' % (imgH)) \
                 + (resize == 'noResize') * ('/noResize/') + (author_id != 'None') * (
                             'single_authors/' + author_id + '/') \
                 + mode + (resize != 'noResize') * (
                             ('_initGap%s' % (init_gap)) * (init_gap > 0) + ('_hGap%s' % (h_gap)) * (h_gap > 0) \
                             + '_NoDiscard_wide' * (not discard_wide) + '_NoDiscard_wide' * (
                                 not discard_narr)) + '_unlabeld' * (not labeled) + \
...
def createDict(label_list, top_dir, dataset, mode, words, remove_punc):
    lex_name = dataset + '_' + mode + (dataset in ['IAM', 'RIMES']) * ('_words' * words) + (dataset == 'IAM') * (
                '_removePunc' * remove_punc)
...
if __name__ == '__main__':
    ...
    dataset = 'ETL_test'  # CVL/IAM/RIMES/gw
    ...
    image_path_list, label_list, outputPath, author_id = create_img_label_list(top_dir, dataset, mode, words,
                                                                               author_number, remove_punc)
    ...
    createDataset(image_path_list, label_list, outputPath, mode, author_id, remove_punc, resize, imgH, init_gap, h_gap,
                  charminW, charmaxW, discard_wide, discard_narr, labeled)
    if create_Dict:
